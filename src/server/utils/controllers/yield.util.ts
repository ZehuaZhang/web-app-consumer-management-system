/**
 * Utility & Helper Functions for YIELD Controller
 */

import { Request } from 'express'
import {
  I_YIELD_Metrics_RequestInput
} from '../../interfaces/controllers/yield.interface'

import { Query } from '../constants.util'

export module Constants {
  export const DefaultMetricsName = 'yield'
  export const DefaultMetricsVersion = '1.0.0'
}

export function getMetricsInputFromRequest(req: Request): I_YIELD_Metrics_RequestInput {
  const query = req.query

  const input: I_YIELD_Metrics_RequestInput = {
